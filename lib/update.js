...
exports.registry = function updateRegistry(release){
  if (!release.get('initialManifest') || release.getIn(['initialManifest', 'private'])) {
   utils.logStepSkipped('(no registry)')
   return null
 } else {
...
function vcsCommit(release){
  if (!release.get('initialManifest')) return Promise.resolve(null)
  return utils
...
exports.manifest = function updateManifest(release){
  return utils.updateJSONFile(utils.getManifestFilepath('npm'), { version: release.get('version') })
}
